...
// @ts-nocheck
/**
 * MediaMTX WebRTC/WHEP 播放器(vendored)。
 *
 * 原文件来自 MediaMTX 官方 `reader.js`,此处原样移植为 ES 模块:去掉 `window.*` 全局
 * 挂载改为 `export`,其余逻辑(SDP 改写、ICE trickle、断线重连)保持不动。作为第三方库
 * 用 `@ts-nocheck` 跳过严格类型检查,仅对外暴露一个带类型的构造配置。
 */

export interface WebRtcReaderConf {
  /** WHEP 端点的绝对地址。 */
  url: string
  /** 观看 token,会以 `Authorization: Bearer` 发送。 */
  token?: string
  user?: string
  pass?: string
